...
| 0.12 | y-axis labels, `social_optimum.png`, `nash_eq.png`, `nash_so_comp.png`, `social_opt_cond.png`, `monopoly.png`, `comparison.png`, `new_comp_fig4x4.png` | Three different wordings across the seven figures: "log population density" (5 figures), bare `ln(l̄)` (`comparison.png`), "log pop. dens." abbreviated (`new_comp_fig4x4.png`) — see `phase0/FINDINGS.md` §3 | **Decided 2026-07-28: standardize on "log population density."** Matches the paper's existing prose (§3.1, Figure 1/2 captions) and 5 of 7 figures already — no text edits needed, only the 2 outlier figure labels. (The appendix's Feb 2026 "labor density" wording is the one to avoid; it would require prose edits too.) |
