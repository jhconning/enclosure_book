...
def teopt(th, alp, c, lbar):
    ...
    # Using global zprime with mu=1 for planner
...
def decomp(te, th, alp, mu, lbar):
    #TODO  not finished ... see notes.
    lam = Lambda(th, alp, mu)
    FTe = (1-alp)*th*lam**alp * (1+(lam-1)*te)**(-alp)
    FTc = (1-alp)*th * (1+(lam-1)*te)**(-alp)
    # dlte = lam/(1+(lam-1)*te)**2
    # FLe = alp*th * (1+(lam-1)*te)**(1-alp)
    # FLc = (1-alp)*th * (1+(lam-1)*te)**(-alp)
...
def plotY(th=1.0, lbar=1.0, alp=0.5, c=1.0, mu=0.0):
    ...
    # teo = teopt(th, alp, c, lbar) # Unused
...
def plotle(te=0.5, th=1.0, alp=0.5, mu=0.5):
    ...
    lemu = le(te, th, alp, mu)
    ax.set_title(r'Labor reaction $l_e(t_e)$')
    ax.set_xlim(0,1)
    ax.set_ylim(0,1)
    ax.set_aspect('equal', 'box')
    ax.plot(tte, le(tte, th, alp, mu=0), linewidth=2, color='b')
    ax.plot(tte, le(tte, th, alp, mu=1), linewidth=2, color='r')  
    ax.plot(tte, le(tte, th, alp, mu), linewidth=2, color='g') 
    ax.plot([0,1],[0, 1],linestyle=':')
    ax.plot([0,te],[0, leq],linestyle='-')
    ax.scatter(te, leop, label=r'Planner $\mu$ =1', color='r')
    ax.scatter(te, leq, label=r'Open Access $\mu$ =0', color='b')
    ax.scatter(te, lemu, label=r'Regulated Commons $\mu$'+f'={mu}', color='g')
    ...
    ax.text(1.15,0.7, r'($\theta$'+f'={th},  '+r'$\alpha$'+f'={alp})', fontsize=11)
...
def plotz(th=1.0, alp=0.5, c=1.0, lbar=DEFAULT_LBAR, ax=None):
...
def plotmpts2(te=0.5, alp=0.5, th=1.0, lbar=DEFAULT_LBAR/DEFAULT_TBAR, mu=0.0):
    ...
    xlabels = ['0', r'$l_e^0(t_e)$', r'$l_e^1(t_e)$','1']
    ...
    ax.set_yticks([])
    ax.text(-0.05, we, r'$w_e$', fontsize=13)
    ax.text(-0.05, wc, r'$w_c$', fontsize=13)
...
# NOTE: Duplicate plotz removed here.
...
def allpart(c=1.0, alp=2/3, mu=0.0, soc_opt=True, cond_opt=True, pv_opt=False, 
            pv_gg=True, logpop=True, ax=None):
    ...
    # ylbl = ax.set_ylabel(r'$\overline{l}$', fontsize=18) # Unused
    ...
    if soc_opt:
        # slocus0 = ax.plot(the_1, lo0, color= 'black', alpha=0.2) # Unused var
        ax.plot(the_1, lo0, color= 'black', alpha=0.2)
        # slocus1 = ax.plot(the_1, lo1, color= 'black', alpha=0.2) # Unused var
        ax.plot(the_1, lo1, color= 'black', alpha=0.2)
        ax.text(ep, np.min(lo0), r'$l^o_0$', fontsize=16)
        ax.text(ep, np.min(lo1)+.05, r'$l^o_1$', fontsize=16)

    if cond_opt:
        # clocus_lo = ax.plot(the_lo, lc,  color='blue', linestyle='dashed') # Unused
        ax.plot(the_lo, lc,  color='blue', linestyle='dashed')
        # clocus0 = ax.plot(the_hi, lc0, color= 'blue', linestyle='dashed') # Unused
        ax.plot(the_hi, lc0, color= 'blue', linestyle='dashed')
        # clocus1 = ax.plot(the_hi, lc1, color= 'blue', linestyle='dashed',linewidth=2.5) # Unused
        ax.plot(the_hi, lc1, color= 'blue', linestyle='dashed',linewidth=2.5)
    
        # t3 = ax.text(ep, np.min(lc0), r'$l^c_0$', fontsize=16) # Unused
        ax.text(ep, np.min(lc0), r'$l^c_0$', fontsize=16)
        # t4 = ax.text(ep, np.min(lc1)-.05, r'$l^c_1$', fontsize=16) # Unused
        ax.text(ep, np.min(lc1)-.05, r'$l^c_1$', fontsize=16)

    if pv_opt:
        # oline1 = ax.plot(the_d, ld0, color= 'red') # Unused
        ax.plot(the_d, ld0, color= 'red')
        # bline1 = ax.plot(the_d, ld1, color= 'red') # Unused
        ax.plot(the_d, ld1, color= 'red')
        # bbline3 = ax.plot(the_gg, ldg, color='red', linestyle='dashed') # Unused
    ...
    # vline1 = ax.axvline((1-(1-alp)*mu)/alp, ymax=.95, linestyle=':', color='black') # Unused
    ax.axvline((1-(1-alp)*mu)/alp, ymax=.95, linestyle=':', color='black')
    # vline2 = ax.axvline(1, ymax=.95, linestyle=':', color='black') # Unused
    ...
    # Ensure plot is shown when not part of a subplot
    if ax is None:
        plt.show()


def threeplots(th, alp, c, lbar=2.0, soc_opt=True, cond_opt=True, pv_opt=False, 
               pv_gg=True, logpop=False):
    '''
    ...
    # top right z(t_e) - c * t_e markers and plot
    ...
    axZ.plot(tte, z(tte, th, alp, lbar) - c*tte, color='blue')        # first best
    axZ.plot(tte, zpv(tte, th, alp, lbar) - c*tte,  color='orange' )      # private
    ...
    # r0 = req(0, th, alp, lbar) # Unused
    # r1 = req(1, th, alp, lbar) # Unused
...
# The function below had undefined variables (teo, lbar, tep, teg). 
